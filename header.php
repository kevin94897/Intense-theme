...
<head>
    ...
    <?php // Fuentes self-hosted: solo se precargan las que se pintan above the fold.
          // La italica y los subsets latin-ext quedan fuera (los baja el navegador solo si hacen falta). ?>
    ...
    <link rel="preload" as="font" type="font/woff2" href="<?php echo esc_url(get_template_directory_uri()); ?>/dist/rubik-latin-normal.woff2" crossorigin>
    <?php
...
<body <?php body_class('bg-cream'); ?>>
    ...
    <header
        ...
        role="banner" x-data="megaMenu()" @keydown.escape.window="closeAll()" @click.outside="closeAll()">

        <!-- ── Top Header (desktop contact bar) ──────────────────── -->
        <div class="top-header <?php echo $is_light_header ? 'bg-dark' : ''; ?>">
            <div class="container-site-wide">
                <div
                    class="flex items-center justify-end py-2 text-sm <?php echo $is_light_header ? 'text-white' : 'text-white'; ?> font-light tracking-wide">
            ...
            <div class="container-site-wide">
                <nav class="flex items-center justify-between py-4" role="navigation"
                    ...
                    <div class="hidden lg:flex items-center gap-8">
                        ...
                        <!-- Selector de regiones (Customize > Selector de Regiones) -->
                        <?php get_template_part('template-parts/components/region-switcher', null, [
                            'color' => $is_light_header ? 'dark' : 'light',
                        ]); ?>
                    </div>
                    ...
                    <button type="button"
                        ...
                        @click="mobileOpen = true" aria-label="<?php esc_attr_e('Abrir menú', 'intense-nerd-theme'); ?>"
        ...
        <div x-cloak x-show="mobileOpen" x-transition:enter="transition ease-out duration-300"
            ...
            class="lg:hidden fixed inset-0 z-[100] bg-cream flex flex-col overflow-hidden" style="display:none;">

            <!-- Fila de iconos de contacto centrados -->
            <div class="flex items-center justify-end gap-5 py-3 border-b border-dark/10 text-dark shrink-0 px-5">
                <a href="tel:<?php echo esc_attr(get_theme_mod('contact_phone', '18006709510')); ?>"
                    aria-label="Teléfono" class="hover:text-primary transition-colors">
                ...
                <span class="h-4 w-px bg-dark/20"></span>
                <a href="mailto:<?php echo esc_attr(get_theme_mod('contact_email', 'sales@intenseperu.com')); ?>"
                    aria-label="Email" class="hover:text-primary transition-colors">
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M4.6155 19C4.15517 19 3.77083 18.8458 3.4625 18.5375C3.15417 18.2292 3 17.8448 3 17.3845V6.6155C3 6.15517 3.15417 5.77083 3.4625 5.4625C3.77083 5.15417 4.15517 5 4.6155 5H19.3845C19.8448 5 20.2292 5.15417 20.5375 5.4625C20.8458 5.77083 21 6.15517 21 6.6155V17.3845C21 17.8448 20.8458 18.2292 20.5375 18.5375C20.2292 18.8458 19.8448 19 19.3845 19H4.6155ZM12 12.1155L4 6.8845V17.3845C4 17.564 4.05767 17.7115 4.173 17.827C4.2885 17.9423 4.436 18 4.6155 18H19.3845C19.564 18 19.7115 17.9423 19.827 17.827C19.9423 17.7115 20 17.564 20 17.3845V6.8845L12 12.1155ZM12 11L19.6923 6H4.30775L12 11ZM4 6.8845V6V17.3845C4 17.564 4.05767 17.7115 4.173 17.827C4.2885 17.9423 4.436 18 4.6155 18H4V6.8845Z"
                            fill="currentColor" />
            ...
            <div class="flex items-center justify-between px-5 py-4 shrink-0">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button type="button" @click="mobileClose()"
                        aria-label="<?php esc_attr_e('Cerrar menú', 'intense-nerd-theme'); ?>"
                        class="flex flex-col justify-center gap-[5px] w-10 h-10 text-dark cursor-pointer">
                        <span
                            class="block w-6 h-[1.5px] bg-current origin-center rotate-45 translate-y-[6.5px] transition-all duration-200"></span>
            ...
            <div class="flex-1 relative overflow-hidden">
                ...
                <div class="absolute inset-0 flex flex-col overflow-y-auto transition-transform duration-300"
                    :class="mobilePanel ? '-translate-x-full' : 'translate-x-0'">
                    <nav class="flex-1 px-6 pt-2">
                        <?php
                        $menu_locations = get_nav_menu_locations();
                        ...
                        if ($menu_items_m):
                            foreach ($menu_items_m as $item_m):
                                ...
                                $panel_key_m = '';
                                if (strpos($classes_m, 'mega-journeys') !== false)
                                    $panel_key_m = 'journeys';
                                elseif (strpos($classes_m, 'mega-destinations') !== false)
                                    $panel_key_m = 'destinations';
                                elseif (strpos($classes_m, 'mega-blog') !== false)
                                    $panel_key_m = 'blog';
                                ?>
                                <div class="border-b border-dark/10">
                                    <?php if ($has_sub): ?>
                                        <button type="button" @click="openMobilePanel('<?php echo $panel_key_m; ?>')"
                                            class="w-full flex items-center justify-between py-5 text-left font-body text-dark text-base hover:text-primary transition-colors cursor-pointer">
                                            <span><?php echo esc_html($item_m->title); ?></span>
                                            <svg class="w-4 h-4 text-dark/40 shrink-0" fill="none" stroke="currentColor"
                                                stroke-width="2" viewBox="0 0 24 24">
                                                <path d="M9 18l6-6-6-6" />
                                            </svg>
                                        </button>
                                    <?php else: ?>
                                        <a href="<?php echo esc_url($item_m->url); ?>" @click="mobileClose()"
                ...
                <!-- VIEW 2: Sub-panel (fixed, cubre todo) -->
                <div class="fixed inset-0 z-[110] bg-cream flex flex-col transition-transform duration-300"
                    :class="mobilePanel ? 'translate-x-0' : 'translate-x-full'">

                    <!-- Sub-panel: ← volver | X cerrar -->
                    <div class="flex items-center justify-between px-5 py-4 shrink-0 border-b border-dark/10">
                        <button type="button" @click="mobilePanel = null"
                            class="text-dark hover:text-primary transition-colors cursor-pointer" aria-label="Volver">
                        ...
                        <button type="button" @click="mobileClose()"
                            ...
                            aria-label="Cerrar">
                            <span
                                class="block w-6 h-[1.5px] bg-current origin-center rotate-45 translate-y-[6.5px]"></span>
                    ...
                    <div class="flex-1 overflow-y-auto px-6 py-6">
                        ...
                        <div x-show="mobilePanel === 'journeys'">
                            <a href="<?php echo esc_url(home_url('/journeys/')); ?>" @click="mobileClose()"
                        ...
                        <div x-show="mobilePanel === 'destinations'">
                            <a href="<?php echo esc_url(home_url('/destinations/')); ?>" @click="mobileClose()"
                                class="block font-body text-dark text-base font-medium mb-6 hover:text-primary transition-colors">
                            ...
                            <div class="grid grid-cols-2 gap-3" id="mob-destination-cards">
                                <?php for ($i = 0; $i < 6; $i++): ?>
                                    <div class="animate-pulse">
                                        <div class="w-full h-[200px] bg-dark/10 rounded-lg mb-1.5"></div>
                        ...
                        <div x-show="mobilePanel === 'blog'">
                            <a href="<?php echo esc_url(home_url('/blog/')); ?>" @click="mobileClose()"
                                class="block font-body text-dark text-base font-medium mb-6 hover:text-primary transition-colors">
            ...
            <!-- Region selector -->
            <?php if (get_theme_mod('region_switcher_enabled', true) && count(intense_get_regions()) > 1): ?>
                <div class="shrink-0 flex items-center justify-between px-6 py-4 border-t border-dark/10 bg-cream">
                    <span class="font-body text-dark/60 text-sm"><?php esc_html_e('Region', 'intense-nerd-theme'); ?></span>
                    <?php get_template_part('template-parts/components/region-switcher', null, [
                        'color' => 'dark',
                        'drop_up' => true,
                        'tooltip' => false,
            ...
            <div class="shrink-0 flex items-center justify-between px-6 py-4 border-t border-dark/10 bg-cream">
                <span
                    class="font-body text-dark/60 text-sm"><?php esc_html_e('Language', 'intense-nerd-theme'); ?></span>
                <?php if (function_exists('icl_get_languages')):
                    $languages = icl_get_languages('skip_missing=0&orderby=code');
                    if (!empty($languages)): ?>
                        <div class="relative" x-data="{ langOpen: false }">
                            <button @click="langOpen = !langOpen"
                                class="flex items-center gap-1.5 font-body text-primary text-sm font-medium cursor-pointer">
                                <?php foreach ($languages as $lang):
                                    if ($lang['active']): ?>
                                        <span><?php echo esc_html($lang['native_name']); ?></span>
                                    <?php endif;
                                endforeach; ?>
                                <svg class="w-3.5 h-3.5 transition-transform" :class="langOpen ? 'rotate-180' : ''" fill="none"
                                    stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                    <path d="M6 9l6 6 6-6" />
                                </svg>
                            </button>
                    ...
                    <?php else: ?>
                        <span class="font-body text-primary text-sm font-medium">English
                            <svg class="inline w-3.5 h-3.5 ml-0.5" fill="none" stroke="currentColor" stroke-width="2.5"
                                viewBox="0 0 24 24">
                                <path d="M6 9l6 6 6-6" />
                            </svg>
                        </span>
                ...
                <div class="flex gap-4 xl:gap-6 2xl:gap-8">
                    ...
                    <div class="flex-1 min-w-0 flex flex-col gap-3">
                        ...
                        <div class="grid grid-cols-3 gap-2 xl:gap-3" id="mega-journey-cards">
                            <?php for ($i = 0; $i < 3; $i++): ?>
                                <div class="animate-pulse">
                                    <div class="w-full h-[180px] xl:h-[240px] 2xl:h-[300px] bg-dark/10 rounded-lg mb-2">
                                    </div>
