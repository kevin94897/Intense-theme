...
// ── Customizer: Selector de Regiones (header) ─────────────────────────────────
const INTENSE_REGION_SLOTS = 3;

function intense_region_defaults()
{
  $flags = get_template_directory_uri() . '/assets/images/flags/';
  return [
    1 => ['label' => 'Intense Perú', 'url' => 'https://intenseperu.com', 'flag' => $flags . 'peru.svg'],
    2 => ['label' => 'Intense Bolivia', 'url' => home_url('/'), 'flag' => $flags . 'bolivia.svg'],
    3 => ['label' => '', 'url' => '', 'flag' => ''],
  ];
}

function theme_customize_region_switcher($wp_customize)
{
  $wp_customize->add_section('region_switcher_section', [
    'title' => 'Selector de Regiones',
    'description' => 'Menú de banderas junto al botón "Design your trip". Deja el nombre vacío para ocultar una región.',
    'priority' => 36,
  ]);

  $wp_customize->add_setting('region_switcher_enabled', ['default' => true, 'sanitize_callback' => 'wp_validate_boolean']);
  $wp_customize->add_control('region_switcher_enabled', [
    'label' => 'Mostrar selector de regiones',
    'section' => 'region_switcher_section',
    'type' => 'checkbox',
  ]);

  $wp_customize->add_setting('region_switcher_tooltip', ['default' => 'Explore our Regions', 'sanitize_callback' => 'sanitize_text_field']);
  $wp_customize->add_control('region_switcher_tooltip', [
  ...
  $choices = [];
  foreach (range(1, INTENSE_REGION_SLOTS) as $i) {
    $choices[$i] = "Región $i";
  }
  $wp_customize->add_setting('region_switcher_current', ['default' => 2, 'sanitize_callback' => 'absint']);
  $wp_customize->add_control('region_switcher_current', [
    'label' => 'Región de este sitio',
    'description' => 'Se muestra en el botón y aparece deshabilitada en la lista.',
    'section' => 'region_switcher_section',
    'type' => 'select',
    'choices' => $choices,
  ]);

  $defaults = intense_region_defaults();
  foreach (range(1, INTENSE_REGION_SLOTS) as $i) {
    $wp_customize->add_setting("region_{$i}_label", ['default' => $defaults[$i]['label'], 'sanitize_callback' => 'sanitize_text_field']);
    ...
    $wp_customize->add_setting("region_{$i}_url", ['default' => $defaults[$i]['url'], 'sanitize_callback' => 'esc_url_raw']);
    ...
    $wp_customize->add_setting("region_{$i}_flag", ['default' => $defaults[$i]['flag'], 'sanitize_callback' => 'esc_url_raw']);
    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, "region_{$i}_flag", [
      'label' => "Región $i — Bandera",
      'description' => 'Imagen cuadrada (se muestra en círculo).',
      'section' => 'region_switcher_section',
    ]));
  }
}
add_action('customize_register', 'theme_customize_region_switcher');

/**
 * Regiones configuradas en el Customizer (solo las que tienen nombre).
 *
 * @return array<int, array{label: string, url: string, flag: string, current: bool}>
 */
function intense_get_regions()
{
  $defaults = intense_region_defaults();
  $current = (int) get_theme_mod('region_switcher_current', 2);
  $regions = [];
  foreach (range(1, INTENSE_REGION_SLOTS) as $i) {
    $label = get_theme_mod("region_{$i}_label", $defaults[$i]['label']);
    if (!$label) {
      continue;
    }
    $regions[$i] = [
      'label' => $label,
      'url' => get_theme_mod("region_{$i}_url", $defaults[$i]['url']),
      'flag' => get_theme_mod("region_{$i}_flag", $defaults[$i]['flag']),
      'current' => $i === $current,
    ];
  }
  return $regions;
  ...
  <tr>
    <td style="background:#423931;padding:26px 40px;text-align:center;">
      <table width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr><td align="center" style="font-size:11px;color:rgba(255,255,255,0.35);line-height:1.7;padding-top:4px;">© Intense Bolivia 2007 – 2026. All rights reserved. Lima, Perú</td></tr>
      </table>
    </td>
  </tr>
HTML;
...
function intense_email_header_html()
{
  ...
  <tr>
    <td style="background:#423931;padding:24px 40px;text-align:center;">
      <img src="{$logo_src}" alt="Intense Bolivia" width="160" style="display:block;margin:0 auto;max-width:160px;height:auto;border:0;">
    </td>
  </tr>
HTML;
}
...
HTML;
  ...
  return intense_email_wrap('New Contact Message — Intense Bolivia', $rows);
}
...
function intense_email_contact_autoreply($data)
{
  ...
  <tr>
    <td style="background:#b76739;padding:32px 40px;text-align:center;">
      ...
      <p style="color:rgba(255,255,255,0.85);font-size:13px;margin:0;line-height:1.5;">Thank you for reaching out to Intense Bolivia</p>
    </td>
  </tr>
  ...
  <tr>
    <td style="padding:36px 40px;background:#ffffff;">
      ...
      <p style="font-size:15px;line-height:1.7;color:#161616;margin:0 0 24px;">While you wait, feel free to explore our signature journeys and destinations — Bolivia has a lot of magic in store for you.</p>
...
HTML;
  ...
  return intense_email_wrap('We received your message — Intense Bolivia', $rows);
}
...
HTML;
  ...
  return intense_email_wrap('New Quote Request — Intense Bolivia', $rows);
}
...
function intense_email_booking_autoreply($data)
{
  ...
  <tr>
    <td style="padding:36px 40px;background:#ffffff;">
      ...
      <p style="font-size:15px;line-height:1.7;color:#161616;margin:0 0 14px;">Dear <strong>{$first_name}</strong>,</p>
      <p style="font-size:15px;line-height:1.7;color:#161616;margin:0 0 20px;">Thank you for trusting Intense Bolivia to plan your journey. We have received your quote request and one of our travel specialists will be in touch within <strong>24 hours</strong> with a personalized proposal.</p>
...
HTML;
  ...
  return intense_email_wrap('Your quote request is on its way — Intense Bolivia', $rows);
}
...
function intense_handle_contact()
{
  ...
  wp_mail(
    $email,
    'We received your message — Intense Bolivia',
...
function intense_handle_booking()
{
  ...
  wp_mail(
    $email,
    'Your quote request is on its way — Intense Bolivia',
...
function intense_handle_brochure()
{
  ...
  wp_mail(
    $email,
    'Your Bolivia Travel Guide — Intense Bolivia',
...
function intense_email_brochure_internal($data)
{
  ...
  <tr>
    <td style="background:#b76739;padding:32px 40px;text-align:center;">
      ...
      <p style="color:rgba(255,255,255,0.85);font-size:13px;margin:0;line-height:1.5;">A visitor requested the Bolivia Travel Guide</p>
    </td>
  </tr>
...
HTML;
  ...
  return intense_email_wrap('New Brochure Download — Intense Bolivia', $rows);
}
...
function intense_email_brochure_autoreply($data)
{
  ...
  <tr>
    <td style="background:#b76739;padding:32px 40px;text-align:center;">
      <h1 style="color:#ffffff;font-size:24px;font-weight:normal;font-family:Georgia,serif;margin:0 0 6px;letter-spacing:0.04em;">Your Bolivia Travel Guide</h1>
  ...
  <tr>
    <td style="padding:36px 40px;background:#ffffff;">
      ...
      <p style="font-size:15px;line-height:1.7;color:#161616;margin:0 0 14px;">Dear <strong>{$first_name}</strong>,</p>
      <p style="font-size:15px;line-height:1.7;color:#161616;margin:0 0 20px;">Thank you for downloading our Travel Guide. We hope it inspires your next adventure. Bolivia has a lot of magic in store for you, and we're here whenever you're ready to start planning.</p>
...
HTML;
  ...
  return intense_email_wrap('Your Bolivia Travel Guide — Intense Bolivia', $rows);
}
...
function intense_email_newsletter_welcome($data)
{
  ...
  <tr>
    <td style="background:#b76739;padding:32px 40px;text-align:center;">
      <h1 style="color:#ffffff;font-size:24px;font-weight:normal;font-family:Georgia,serif;margin:0 0 6px;letter-spacing:0.04em;">Welcome to Intense Bolivia</h1>
  ...
  <tr>
    <td style="padding:36px 40px;background:#ffffff;">
      ...
      <p style="font-size:15px;line-height:1.7;color:#161616;margin:0 0 14px;">Dear <strong>{$first_name}</strong>,</p>
      <p style="font-size:15px;line-height:1.7;color:#161616;margin:0 0 20px;">Thank you for subscribing to Intense Bolivia. You'll be among the first to receive <strong>curated travel inspiration</strong>, exclusive journey highlights, and insider stories from the heart of Peru.</p>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
        <tr><td style="background:#fcf1eb;border-left:3px solid #b76739;padding:16px 20px;">
          ...
          <div style="font-size:14px;color:#161616;line-height:1.8;">
            ...
            · Destination spotlights — from La Paz to the Uyuni Salt Flats<br>
      ...
      <p style="font-size:15px;line-height:1.7;color:#161616;margin:0 0 24px;">In the meantime, explore our collection of private journeys — each one designed to reveal the real Bolivia at your own pace.</p>
...
HTML;
  ...
  return intense_email_wrap('Welcome to Intense Bolivia', $rows);
}
...
function intense_newsletter_subscribe()
{
  ...
  wp_mail(
    $email,
    'Welcome to Intense Bolivia — You\'re in!',
...
// ── Tipografias ───────────────────────────────────────────────────────────────
// Rubik ya no viene de Google Fonts: se sirve self-hosted desde el tema.
// Los @font-face viven en src/css/app.css y salen compilados en dist/main.css,
// asi evitamos la cadena HTML -> CSS de googleapis -> woff2 de gstatic.