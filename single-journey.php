...
if (!is_wp_error($journey_cats) && !empty($journey_cats)) {
    ...
    while ($hotel_query->have_posts()) {
        ...
        $hotels_data[] = [
            'id' => $h_id,
            'image' => get_the_post_thumbnail_url($h_id, 'large') ?: get_template_directory_uri() . '/assets/images/intense_01.webp',
            'title' => get_the_title(),
    ...
    while ($act_query->have_posts()) {
        ...
        $activities_data[] = [
            'id' => $act_id,
            'image' => get_the_post_thumbnail_url($act_id, 'large') ?: get_template_directory_uri() . '/assets/images/intense_01.webp',
            'title' => get_the_title(),
...
<main id="primary" class="site-main">
    ...
    <?php endif; ?>
    <?php
    // El cuerpo se muestra aunque aún no haya itinerario día a día, si hay hoteles o actividades.
    $has_tours = $itinerary_group && !empty($itinerary_group['list_of_tours']);
    if ($has_tours || !empty($hotels_data) || !empty($activities_data)): ?>
