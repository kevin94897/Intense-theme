 ...
 *     │     ├── destinations_list (repeater, max 9)
        ...
        <section class="py-10 bg-cream">
            <div class="container-site text-left">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-2 auto-rows-[300px]">
                    <?php
                    // Bento de 9 destinos en 3 filas de 4 columnas: 1-2-1 / 2-1-1 / 1-1-2.
                    $bento_classes = [
                    ...
                    foreach ($dest_list as $idx => $dest_item):
                        ...
                        // Sin imagen en la fila: se usa la destacada del destino enlazado.
                        $img_array = $dest_item['destination_image'] ?? [];
                        $dest_post_id = !empty($dest_btn['url']) ? url_to_postid($dest_btn['url']) : 0;
                        $thumb = $img_array['sizes']['large'] ?? $img_array['url'] ?? '';
                        if (!$thumb && $dest_post_id) {
                            $thumb = get_the_post_thumbnail_url($dest_post_id, 'large') ?: '';
                        }
                        if (!$thumb) {
                            $thumb = get_template_directory_uri() . '/assets/images/intense_01.webp';
                        }
                        ...
                        <div class="<?php echo $bento_classes[$idx]; ?> relative group overflow-hidden rounded-sm cursor-pointer"
                            data-aos="fade-up" data-aos-delay="<?php echo 200 + ($idx % 4) * 100; ?>">
            ...
            <div class="flex flex-col-reverse md:flex-row items-center gap-12">
                ...
                <div class="w-full md:w-1/2 text-center md:text-left" data-aos="fade-left">
                    <h3 class="font-heading text-3xl font-semibold text-dark mb-4">
                        <?php echo esc_html($guides_title ?: 'Intense Bolivia Travel Guides'); ?>
                    </h3>
    ...
    <div x-data="brochureModal()" @ccp:openbrochure.window="show = true" x-show="show" style="display:none;"
        ...
        <div x-show="show" @click.outside="show = false" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="hidden md:block w-full md:w-5/12 h-64 md:h-auto relative p-5">
                ...
                ?>
                <img src="<?php echo esc_url($modal_img_url ?: $fallback_img); ?>" alt="Bolivia Travel Guide"
